...
//        viewModel.getNavigationLogs()
    ...
    private fun observe() {
        with(viewModel) {
            internet.observe(this@SplashActivity) { internet ->
                if (internet) {
    ...
    private fun observeScreenTransaction() {
        with(viewModel) {
            isUserAssigned.observe(this@SplashActivity) { isUserAssigned ->
    ...
    private fun noInternetScenario() {
        ...
        Timber.e("noInternetScenario 7 ${viewModel.hasOverlayPermission(this)} ")
    ...
    private fun runKiosk() {
        ...
        lifecycleScope.launch {
            if (viewModel.isDeviceOwner()) {
                viewModel.runKiosk(this@SplashActivity) { result, error ->
                    if (result != null) {
                        handler.post {
                            when (result) {
